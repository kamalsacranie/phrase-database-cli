...
def submit_add_new_table(
    ...
    window = all_tables_view(manager, window)


def submit_add_new_entry(
    manager: ptg.WindowManager, window: ptg.Window, table: Table | str
) -> None:
    form_data = {}
    for widget in window:
        if isinstance(widget, ptg.InputField):
    ...
    window = table_view(manager, window, table)  # type: ignore
...
def add_entry_view(
    manager: ptg.WindowManager, window: ptg.Window, table: Table | str, *args
) -> None:
    manager.remove(window)
    window = ptg.Window(
        *args,
...
def table_view(
    ...
    window = ptg.Window(
        ...
        ptg.Button(
            "Add entry",
            lambda *_: add_entry_view(manager, window, table),
        ),
        width=200,
        box="DOUBLE",
    )
...
def all_tables_view(manager: ptg.WindowManager, window: ptg.Window):
    manager.remove(window)
    tables = c.get_tables()
    table_buttons = [
        ptg.Button(
            table.name,
            lambda button: table_view(manager, window, button.label),
        )
        for table in tables
    ]
    window = ptg.Window(
...
def start_tui():
    with ptg.WindowManager() as manager:
        window = ptg.Window(
            ptg.Button(
                "Start", onclick=lambda *_: all_tables_view(manager, window)
            ),
            width=60,
            box="DOUBLE",
        )
